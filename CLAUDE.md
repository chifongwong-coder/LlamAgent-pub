...
LlamAgent is a modular AI Agent framework. The core `LlamAgent` is a standalone chatbot; each pluggable module adds a capability (tools, retrieval, memory, reasoning, reflection, multi-agent, MCP, safety, skill, sandbox, child agent, job, compression, persistence).
...
│   ├── modules/              # 14 pluggable modules + shared infrastructure
...
- Persistence module (v2.3.1): `on_output` saves conversation history (excluding current turn); `on_shutdown` fallback save; JSON format per persona; `persistence_enabled` / `persistence_auto_restore` / `persistence_dir`

## Testing

- **Public tests** (`tests/`): curated flow tests (public, tracked by git, used by CI) — 95 tests
  ...
  - `test_v2_features.py` (43) — v2.0 (mode config, abort, runner, triggers, streaming) + v2.1 (per-module model, compression) + v2.2 (FS parser, FSStore, FSMemoryStore, retrieval/memory backend switching) + v2.3 (reflection mode/FS/tools) + v2.3.1 (persistence save/restore/config)
  - Run: `llamagent_env/bin/python -m pytest tests/ -v`
- **Internal tests** (`tests_internal/`): detailed tests (private, gitignored) — 526 mock + 14 real
  ...
  - v2.0-v2.3.1: `test_v2_features_mock.py`
...
- **Total: 635 tests** (95 public + 526 internal mock + 14 real)
