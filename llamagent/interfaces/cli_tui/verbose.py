...
def install_verbose(agent, target: "Widget") -> None:
    ...
    def _has_real_method(client, name: str) -> bool:
        ...
        Defensive guard against a future outermost LLM that exposes
        ``chat`` / ``chat_stream`` only through ``__getattr__``
        forwarding — wrapping a proxied attribute would install
        successfully but have no clear contract about which call path
        in the underlying client actually hits our wrapper. Production
        stack today (ResilientLLM / LoggingLLM) defines real methods,
        so the guard is purely future-proofing. MRO walk is required
        because subclasses inheriting a method from a base — e.g.,
        ResilientLLM inheriting ``chat_stream`` from LLMClient — count
        as real owners.
        """
