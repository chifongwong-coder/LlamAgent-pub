"""
LlamAgent legacy CLI (input() + rich.console).

This is the pre-TUI command-line interface. ``cli.py`` is now a thin
router that picks between this module and the Textual TUI based on
``--legacy`` / terminal capability; everything below was moved here
1:1 from the old ``cli.py`` so the legacy path keeps working byte-for-byte.

Entry points (called from the router or from ``llamagent.main``):
- ``legacy_main(args)``: full interactive / direct-mode entry, including the
  ``ask`` subcommand and the setup-restart loop.
- ``run_cli(agent)``: chat loop on a prebuilt agent — used by
  ``llamagent.main --mode cli`` and re-exported through ``cli.py``.
- ``LlamAgentCLI``: the chat / slash-command class still available for
  callers that drive the loop themselves (mostly tests + legacy scripts).
- ``build_agent`` / ``interactive_setup`` / ``PRESETS``: same factory and
  prompts the TUI's ``cli_tui.__main__.build_agent_from_setup`` mirrors.
...
def _install_verbose_tracing(agent):
    ...
    time. Activated only from the legacy CLI path (Web/API have their
    own UX; the TUI has /verbose pane); no-op when cli_verbose is False.
...
def legacy_main(args) -> None:
    """Legacy CLI entry point: interactive setup loop or direct mode.

    Accepts a pre-parsed ``argparse.Namespace`` from the router (cli.py)
    so the legacy module no longer owns the parser. Field expectations
    (mirroring the v3.9.0 parser the router preserves):

    * ``args.modules: str | None`` — comma-separated list, or None
    * ``args.no_modules: bool``
    * ``args.command: str | None`` — "chat", "ask", or None
    * ``args.question: str`` — when command == "ask"
    * ``args.output_format: str`` — "text" / "json"
    """
    if not HAS_RICH:
        print(
